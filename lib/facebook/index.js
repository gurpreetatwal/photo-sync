...
fb.getPeople = async function(token) {
  ...
  res = await request.get(
    `/${res.data.id}/taggable_friends?access_token=${token}`,
  );
  return res.data;
};
...
router.get('/contacts', async (ctx, next) => {
  ...
  const auth = await knex('facebook')
    .where({id: ctx.session.facebook})
    .first();

  // TODO check expiration and reauth
  ctx.body = await fb.getPeople(auth.access_token);
});
